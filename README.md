...
The development version requires macOS 27 or later. Older releases retain the
requirements listed in their release notes. Download the ZIP for your Mac from the
...
- **Native appearance controls.** Choose System, Light, or Dark and a more
  translucent sidebar in Settings → Appearance. Changes apply across windows
  immediately and are remembered between launches.
...
Requires macOS 27, Swift 6.4, and the macOS 27 SDK from Xcode 27 or its
standalone Command Line Tools. To select a different Xcode installation,
set `DEVELOPER_DIR` to its `Contents/Developer` directory.

Native view state uses `@ViewState`, a type alias for the public
`SwiftUI.State` property wrapper. This preserves SwiftUI state and bindings
without requiring the newer `SwiftUIMacros` plugin omitted from standalone
Command Line Tools.
...
The packaging script uses the selected developer tools' default SDK and prints
its path. Set `SDKROOT` to an SDK name (such as `macosx27.0`) or an absolute SDK
path to choose explicitly. SDKs older than macOS 27 are rejected.

The current standalone Command Line Tools' SwiftBuild engine can emit linker
warnings about missing `CommandLineTools/Developer/Library/Frameworks` and
`CommandLineTools/Developer/usr/lib` paths. Those testing search paths are
generated by the toolchain; the actual resources are under
`CommandLineTools/Library/Developer`. They do not prevent building or signing
Lexicon. The build preserves linker diagnostics; selecting a complete Xcode
installation uses its platform layout instead.

CI uses GitHub's [`xcode-27` arm64 runner](https://github.blog/changelog/2026-09-10-xcode-27-runner-image-now-runs-on-macos-27/),
currently in public preview. It builds both debug and release configurations
with warnings treated as errors, runs parser, translation, tab, and WebKit
regressions, and validates the app bundle.
...
1. Update `CHANGELOG.md`, then run the complete release checks:
   ...
   swift run -c release -Xswiftc -warnings-as-errors MdxKitTester
   swift run -Xswiftc -warnings-as-errors Lexicon --tab-state-test
   swift run -c release -Xswiftc -warnings-as-errors Lexicon --tab-state-test
...
**Settings → Appearance**, immediately after General, contains:

- **Theme:** System (the default), Light, or Dark. System follows macOS;
  explicit choices apply to all windows, Settings, and sheets.
- **Translucent sidebar:** enabled by default, with native blur that reveals
  more of the desktop behind the window. Turn it off for a solid sidebar.
  macOS accessibility settings can also replace the effect with a solid
  background while keeping your saved preference.

Both settings update open windows immediately and persist across launches.
**General → Restore All Defaults…** restores System and enables sidebar
translucency. Theme changes preserve your open entries and reading position;
each dictionary continues to use its own styles.
...
Tests use a small standalone runner without requiring XCTest or swift-testing.
The MdxKit parser suite works with Command Line Tools; app tests need the
complete SwiftUI toolchain described above. Parser tests validate against fixture
