...
Requires macOS 27, Swift 6.4, and the macOS 27 SDK. Use a complete Xcode 27
installation, with `DEVELOPER_DIR` set to its `Contents/Developer` directory
if it is not already selected. Standalone Command Line Tools may omit
`SwiftUIMacros`, which the macOS 27 SDK's SwiftUI `@State` requires. If the
compiler reports that plugin missing, select the complete Xcode installation.
