...
# Let Apple's encoder choose the correct payload format for each size,
# including the small 1x representations that cannot use our old PNG packer.
iconutil -c icns "$ICONSET" -o "$TEMP_DIR/Lexicon.icns"
mv "$TEMP_DIR/Lexicon.icns" "$OUTPUT"
echo "Built $OUTPUT"
